...
// ─────────────────────────────────────────────────────────────────────
// DevTestPanel — TEMP. Phase 1 Step 5d.2.b sim-review aid.
//
// Drives the backend seed-state harness (internal/booking/dev_seed.go)
// so we can fast-forward an existing booking across the two-OTP
// lifecycle without manually walking accept -> en-route -> arrive ->
// Start OTP -> in_progress -> pay -> End OTP. The harness emits the
// same FCM data push the real path emits, so both apps refresh
// exactly like they would in pilot.
//
// Workflow:
//   1. Sign in on the customer app, create a booking via Cart.
//   2. Tap "Seed pro" once — upserts the test helper account and
//      shows the phone to sign in to the pro app with.
//   3. Tap a state button — backend transitions the booking +
//      issues the relevant Redis OTP + emits booking_status_change
//      pushes. Customer + pro screens auto-flip.
//
// REMOVE before final 5d push. Grep target: "DevTestPanel". One
// component, one styles block, env-driven seed key — clean delete.
// ─────────────────────────────────────────────────────────────────────

// DEV_SEED_KEY matches backend .env.local DEV_SEED_KEY. Hardcoded
// here because this whole component lives behind a __DEV__ gate and
// is removed before pilot. If the backend operator changes the key
// in their .env.local, edit it here too.
const DEV_SEED_KEY = 'dev-seed-local';

type SeedState = 'en_route' | 'in_progress' | 'cash_resolved' | 'online_resolved';

function DevTestPanel() {
  const nav = useNavigation<NativeStackNavigationProp<MainStackParamList>>();
  const { token } = useAuth();
  const [liveId, setLiveId] = React.useState<string | null>(null);
  const [liveHelper, setLiveHelper] = React.useState<string | undefined>();
  const [busy, setBusy] = React.useState(false);

  // Refresh on focus + initial load so creating a fresh booking on
  // the same session enables the state buttons without app restart.
  const loadLive = React.useCallback(async () => {
    if (!token) return;
    try {
      const { getBookings } = await import('../../api/bookings');
      const list = await getBookings(token, 'upcoming');
      const best =
        list.find(b => b.status === 'in_progress')
        ?? list.find(b => b.status === 'accepted' && !!b.helper_id)
        ?? list[0];
      if (!best) {
        setLiveId(null);
        return;
      ...
      setLiveId(best.id);
      setLiveHelper(best.helper_name);
    } catch {
      // swallow — dev panel only.
    }
  }, [token]);
  ...
  const seedHelper = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/dev/seed-helper`, {
        method: 'POST',
        headers: { 'X-Dev-Seed-Key': DEV_SEED_KEY },
      });
      if (!res.ok) {
        showError(`seed-helper failed (${res.status})`);
        return;
      }
      const body = (await res.json()) as { helper_phone: string; otp_code: string };
      Alert.alert(
        'Test pro account ready',
        `Sign in to the pro app:\n\nPhone: ${body.helper_phone}\nOTP: ${body.otp_code}`,
        [{ text: 'OK' }],
      );
    } catch (e: any) {
      showError(`seed-helper crashed: ${String(e?.message ?? e)}`);
    } finally {
      setBusy(false);
    }
  };

  const seedState = async (state: SeedState) => {
    if (busy) return;
    if (!liveId) {
      showError('No booking to attach to — create one via Cart first.');
      return;
    }
    setBusy(true);
    try {
      const { BASE_URL } = await import('../../api/config');
      const res = await fetch(`${BASE_URL}/dev/bookings/${liveId}/seed-state`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        showError(`seed-state ${state} failed (${res.status}): ${body.slice(0, 120)}`);
        return;
      }
      const body = (await res.json()) as { helper_phone: string };
      showSuccess(`Booking -> ${state}`);
      // Jump straight to TrackLive so we can see the resulting state.
      nav.navigate('TrackLive', {
        bookingId: liveId,
        helperName: liveHelper ?? body.helper_phone,
      });
    } catch (e: any) {
      showError(`seed-state ${state} crashed: ${String(e?.message ?? e)}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <View pointerEvents="box-none" style={devPanelStyles.wrap}>
      <View style={devPanelStyles.panel}>
        <Text style={devPanelStyles.label}>DEV · 5d.2.b</Text>

        <TouchableOpacity
          style={devPanelStyles.btnSetup}
          onPress={seedHelper}
          disabled={busy}
        >
          <Text style={devPanelStyles.btnSetupText}>Seed pro</Text>
        </TouchableOpacity>

        {(['en_route', 'in_progress', 'cash_resolved', 'online_resolved'] as SeedState[]).map(
          s => (
            <TouchableOpacity
              key={s}
              style={[devPanelStyles.btn, (busy || !liveId) && devPanelStyles.btnDisabled]}
              onPress={() => seedState(s)}
              disabled={busy || !liveId}
            >
              <Text style={devPanelStyles.btnText}>{s.replace('_', ' ')}</Text>
            </TouchableOpacity>
          ),
        )}

        <TouchableOpacity
          style={devPanelStyles.btnRefresh}
          onPress={loadLive}
          disabled={busy}
        >
          <Text style={devPanelStyles.btnRefreshText}>↻ pick booking</Text>
        </TouchableOpacity>

        {!liveId && (
          <Text style={devPanelStyles.hint}>book via Cart{'\n'}then ↻</Text>
        )}
      </View>
    </View>
  );
...
const devPanelStyles = StyleSheet.create({
  wrap: {
    position: 'absolute',
    bottom: 200,
    right: 0,
  ...
  btn: {
    ...
    borderColor: 'rgba(245,163,0,0.34)',
    borderWidth: 1,
    borderRadius: 8,
    alignItems: 'center',
  },
