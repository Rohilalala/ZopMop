...
--
-- Restores the exact pre-112 state: removes the 4 newly-inserted services and
-- restores all 18 original service_categories rows to their canonical (025)
-- values — reversing the renames, the 30/60/90 duration/order changes, and the
-- deactivations. Enables a local up→down→up verification pass.

-- ── 1. Remove the 4 services created by 112 ───────────────────────────────────
DELETE FROM service_categories WHERE id IN (
    ...
    'a1000000-0000-0000-0000-000000000022'  -- Pre and Post Party Clean
);

-- ── 2. Restore the 18 original rows to canonical (025) values ──────────────────
