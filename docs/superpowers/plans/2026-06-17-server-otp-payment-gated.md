...
Create `App/househelp-api/migrations/144_booking_otp.up.sql` (CSPRNG via pgcrypto — hardened after the commit security review flagged plain `random()` as a weak primitive):
```sql
