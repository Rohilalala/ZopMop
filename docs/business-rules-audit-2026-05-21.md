...
|---|---|---|---|
| C1 | ~~Two pro-pay formulas alive~~ **RESOLVED 2026-06-14 (product sign-off).** Single time-based model: pay = online_minutes × ₹80/hr + working_minutes × ₹80/hr (a working hour pays ₹160). Working-minutes credit the **booked** job duration (not actual time on site), capped at online time (working ⊆ online). Per-booking piece-rate + peak + weekend **deleted** (`booking/earnings.go` removed); the `pro_earnings_paise` snapshot is no longer written/shown. `payroll/calc.go ComputePay` is the sole pay rule and now **caps** working at online instead of skipping. Offer estimate = booked × ₹160/hr. | `internal/payroll/calc.go` (sole formula); `internal/booking/service.go` (credits booked job_minutes) | Done. |
...
| C7 | **Aadhaar + bank account stored plaintext**. Phase 12 TODO. _2026-06-14: access-control half mitigated — worker detail now ships only last-4-masked values; unmasked reveal is a superadmin-only, audited endpoint (`GET /workers/:id/pii` → `worker.pii.reveal`). **Still OPEN: encryption-at-rest (KMS).**_ | `internal/crm/workers/repository.go:506-510`, `model.go:59-67`; mask+reveal in `workers/handler.go` | Access-control done. Encrypt before pilot if any real KYC collected. |
